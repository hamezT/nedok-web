...
/**
 * Hook for subscribing to gateway measurement and upload interval status
 *
 * @example
 * ```typescript
 * const { data, loading, isConnected } = useGatewayMeasurementStatus(
 ...
 * useEffect(() => {
 *   if (data['device-id-1']) {
 *     console.log('Device 1 measurement:', data['device-id-1'].measurement_started);
 *   }
 * }, [data]);
 * ```
 */
export const useGatewayMeasurementStatus = (
  deviceIds: string[],
  enabled: boolean = true
) => {
  ...
  const { isConnected } = useWebSocket({
    autoConnect: enabled && deviceIds.length > 0,
  });

  const handleStatusUpdate = useCallback((newData: Record<string, Record<string, any>>) => {
    setData(prev => ({ ...prev, ...newData }));
    setLoading(false);
  }, []);

  useEffect(() => {
    if (!enabled || deviceIds.length === 0 || !isConnected) {
      // Unsubscribe from all
      if (subscriptionIdsRef.current.length > 0) {
        import('../services/deviceService').then(({ unsubscribeFromGatewayMeasurementAndUploadIntervalStatus }) => {
          unsubscribeFromGatewayMeasurementAndUploadIntervalStatus(subscriptionIdsRef.current);
        });
    ...
    setLoading(true);
    import('../services/deviceService').then(({ subscribeToGatewayMeasurementAndUploadIntervalStatus }) => {
      const ids = subscribeToGatewayMeasurementAndUploadIntervalStatus(deviceIds, handleStatusUpdate);
  ...
  }, [deviceIds, enabled, isConnected, handleStatusUpdate]);
